...
# Using day_of_the_week and MONTH function. Časové proměnné binární proměnné pro víkend / pracovní den roční období daného dne (zakódovat jako 0 až 3)
...
CREATE OR REPLACE VIEW v_covid_variables AS
 SELECT cbd.`date`,
        ...
        CASE WHEN (MONTH (cbd.`date`) IN ('3') AND CONVERT(DAYOFMONTH(cbd.date),INT) >= 20) OR (MONTH(cbd.`date`) IN ('4','5'))
               OR (MONTH (cbd.`date`) IN ('6') AND CONVERT(DAYOFMONTH(cbd.date),INT) <= 20) THEN 0
             WHEN (MONTH (cbd.`date`) IN ('6') AND CONVERT(DAYOFMONTH(cbd.date),INT) >= 21) OR (MONTH (cbd.`date`)IN ('7','8'))
               OR (MONTH (cbd.`date`) IN ('9') AND CONVERT(DAYOFMONTH(cbd.date),INT) <= 21) THEN 1
             WHEN (MONTH (cbd.`date`) IN ('9') AND CONVERT(DAYOFMONTH(cbd.date),INT) >= 22) OR (MONTH(cbd.`date`) IN ('10','11'))
               OR (MONTH (cbd.`date`) IN ('12')AND CONVERT(DAYOFMONTH(cbd.date),INT) <= 20) THEN 2
             WHEN (MONTH (cbd.`date`) IN ('12')AND CONVERT(DAYOFMONTH(cbd.date),INT) >= 21) OR (MONTH(cbd.`date`) IN ('1','2'))
               OR (MONTH (cbd.`date`) IN ('3') AND CONVERT(DAYOFMONTH(cbd.date),INT) <= 19) THEN 3 
             END AS season_code           
...
#Pomocí Select Distinct, zjistím kolik celkem náboženství se vyskytuje ve sloupci religion a pro každé náboženství vytvořím sloupec, který bude výsledkem porovnání s celkovou populací dané zemì.
...
#Calculation of life_expectancy. VARIABLE life_exp_ratio - rozdíl mezi očekávanou dobou dožití v roce 1965 a v roce 2015.
...
#missing countries- země, které nejsou uvedeny buď v žádné tabulce nebo jen v covid19_basic_differences
...
#from religions r;
  ...
  #ve finální tabulce je zastoupeno pouze 16 zemí, protože potřebujeme data jak pro počty nakažených, tak pro počet testovaných. V opačném případě je pozorování zkreslené.
  #select distinct country
